...
> **Note:** Approximate rate limits are ~50 messages/second per connection and ~5 connections per IP (market-data and trading combined). Monitor for disconnects and back off if you are throttled.
...
> **Note:** The signature timestamp must be within ±30 seconds of server time, and each nonce is single-use within the validity window. Generate a fresh `timestamp` and `nonce` for every sign-in — including on every reconnect.
...
### Command Status (`mt: 3`) {#command-status-mt-3}
