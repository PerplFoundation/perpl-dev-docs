...
> **Note:** The signing scheme includes the network's numeric **chain ID**, so you must sign with the value for the network you are calling. The two live networks are Mainnet (chain ID `143`, default) and Testnet (chain ID `10143`). Full endpoint, contract, and market details are on the [Networks](../getting-started/networks.md) page — reuse those values rather than hard-coding your own.
...
> **Note:** The REST base URL includes the `/api` suffix; the WebSocket URL does **not**.
...
> **Note:** **Withdrawals and transfers-out are never permitted via an API key, regardless of scope.** Those actions require a wallet signature on the Exchange contract directly.
...
To trade you must additionally create an on-chain account with initial collateral by calling `createAccount(uint256 amountCNS)` on the Exchange contract. Until that account exists, some authenticated calls return **404**. See the [Networks](../getting-started/networks.md) page for the Exchange contract address per network, and the account-creation walkthrough for the `cast` commands.
...
> **Note:** Listing and revoking keys is done from the web UI (`/apikeys`), **not** via the API. A revoked public key cannot be re-enrolled — generate a fresh key pair.
...
2. **Request** an EIP-712 payload from `/api-key/payload`.
3. **Sign** it with two signatures: the user's wallet (proves account ownership) and the API key (proves you hold its private key).
4. **Enroll** via `/api-key/enroll`, echoing the payload back and submitting both signatures. The response contains the opaque `X-API-Key` token.

> **Note:** Both enrollment endpoints are CORS-enabled (CORS = Cross-Origin Resource Sharing), so they can be called from your backend **or** directly from browser code. In both cases the request `Origin` must be pre-whitelisted by Perpl (it is recorded as `ApiKeyInfo.origin`). A browser sets `Origin` automatically; from a server you must set it explicitly. Ask Perpl to whitelist the origin(s) you enroll from.
...
> **Note:** The `request-target` must match byte-for-byte what the server receives — include the full query string (`?count=100&page=...`) exactly as sent, in the same order. Any mismatch changes the canonical string and the signature will be rejected.
...
> **Note:** `X-API-Timestamp` and `X-API-Nonce` must be the **same** values you placed into the canonical string. Re-generate both (and re-sign) for every request.
...
> **Note:** The market-data WebSocket (`/ws/v1/market-data`) requires no authentication — connect and subscribe directly.
