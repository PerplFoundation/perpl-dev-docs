...
- **[Direct REST + WebSocket API](#path-a-rest--websocket-api)** — call the HTTP and WebSocket endpoints from any language.
- **[Rust SDK](#path-b-rust-sdk)** — use the `perpl-sdk` crate and the `perpl-cli` command-line tool.
...
The full network reference lives in [Networks](./networks.md).

## Two ways to integrate

Perpl exposes the same exchange through two integration surfaces. Pick whichever fits your stack.

### Path A: REST + WebSocket API
...
| [Networks](./networks.md) | Full mainnet and testnet reference: URLs, chain IDs, contract addresses, tokens. |
| [API Guide](../api/rest.md) | REST endpoints, WebSocket streams, and API-key authentication (any language). |
| [SDK Guide](../sdk/install.md) | The `perpl-sdk` crate and `perpl-cli`: state cache, event streams, order posting. |
